...
<v-app-bar-title class="font-weight-bold">
  ...
  ALIVE
    ...
    <v-col   :style="{width: searchBoxClosed ? '65%' : '100%', flex: searchBoxClosed? '0 0 65%':'0 0 100%'}"  sm="8" md="4" lg="4"  >
      ...
      :class="{ closed: searchBoxClosed && !searchText }" @keyup.enter=""
      v-model="searchText"
      @focus="searchBoxClosed = false"
      @blur="searchBoxClosed = true" absolute></v-text-field>
        <v-menu>
          <template v-slot:activator="{ props }">
            <v-btn flat icon="mdi:mdi-filter-variant " v-bind="props">
            </v-btn>
          </template>
        ...
        <v-btn icon="mdi:mdi-history" flat></v-btn>
      </v-row>
    </v-col>
