<template>
  <v-card v-if="data" class="rounded-0 mb-2 fill-height" density="compact" variant="elevated" elevation="2" rounded="lg">

    <v-img :lazy-src="data.urlToImage ? data.urlToImage : placeholderImage" class="rounded-lg"
      :src="data.urlToImage ? data.urlToImage : placeholderImage" height="200" cover
        ...
        <template v-slot:append>
          <v-btn icon variant="text" color="white">
            ...
            <v-menu activator="parent">
              <v-list rounded="xl">
                <v-list-item value="detail" key="detail" prepend-icon="mdi:mdi-page-next" :to="{
                  name: 'article.show',
                  params: {
                    id: data.id,
                  },
                ...
                <v-list-item value="edit" key="edit" prepend-icon="mdi:mdi-pencil-box"
                ...
                <v-list-item value="goToSource" key="goToSource" prepend-icon="mdi:mdi-link-variant" :href="data.url"
                  target="_blank">
                  Original Source
                </v-list-item>
              </v-list>
    ...
    <v-card-title v-if="highlightText" v-html="formatHighlightText(processedContent( data.title).substring(0, 30))" class="pb-0 overflow-hidden">
    </v-card-title>
      ...
      <router-link class="text-decoration-none text-black" :to="{
        ...
        <v-tooltip :text="data.title" location="top">
          <template v-slot:activator="{ props }">
            ...
            <span v-bind="props" class="overflow-hidden pb-0"> {{ processedContent( data.title).substring(0, 50) }}...</span>
          </template>
        </v-tooltip>
    ...
    <v-card-text class="text-caption pb-0">
      ...
      <v-chip class="ma-2" size="small" :to="{
        name:'home.search',
        query:{
    ...
    <v-card-text>
      <span v-if="highlightText" v-html="formatHighlightText(processedContent(data.description).substring(0, 50))"></span>
      <span v-else v-html="processedContent(data.description).substring(0, 50)+'...'">
...
return content ? content.replace(/<a href="(.*?)">(.*?)<\/a>/g, '<a :href="$1">$2</a>') : "";
