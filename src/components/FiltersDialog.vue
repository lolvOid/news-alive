<template>
    <v-dialog v-model="dialog" scrollable rounded="xl" :width="display ? '400' : '100%'" :fullscreen="!display"
        :scrim="false" persistent>
        <v-card>
            ...
            <v-text-field variant="solo" flat label="Search Sources" prepend-inner-icon="mdi:mdi-magnify" single-line
                hide-details v-model="searchedSource"  clearable autofocus></v-text-field>
            <v-divider></v-divider>

            <v-card-text :style="{ 'height': display ? '350px' : 'auto' }">
                <v-list>
                    <v-list-item v-model="selected" :prepend-avatar="s.icon" v-for="(s, id) in filteredSources" :key="s.id"
                        :value="s.id" @click="updateSelectedSource(s.id)">
